<?php
/*
Template Name: Resources
*/
?>
<?php get_header(); ?>
<section class="py-5 resources-section">
    <div class="container-fluid">
        <div class="row">
            <!-- left column  -->
            <div class="col-lg-9 px-lg-5">
                <div class="row justify-content-center mb-5">
                    <div class="col-12 text-center mb-4">
                        <h2 class="fw-bold font-xl">Start Your Journey Here</h2>
                        <div class="underline mx-auto"></div>
                    </div>
                </div>

                <div class="row gap-5 justify-content-center flex-wrap">
                    <!-- Card 1 -->
                    <div class="col-lg-5 resource-card-main">
                        <div class="resource-card  py-3 px-3 py-lg-4 px-lg-4 position-relative d-flex gap-2 gap-lg-5">
                            <div class="my-auto content-col">
                                <h5 class="text-blue fw-bold text-capitalize">Family & Caregivers</h5>
                                <p>Care for the caregiver</p>
                            </div>
                            <div class="my-auto me-5">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/family.png" class="float-end rounded" alt="">
                            </div>
                            <div class="corner-badge">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/Monitor-Heart-Beat.png" alt="">
                            </div>
                        </div>
                    </div>
                    <!-- Card 2 -->
                    <div class="col-lg-5 resource-card-main">
                        <div class="resource-card  py-3 px-3 py-lg-4 px-lg-4 position-relative d-flex gap-2 gap-lg-5">
                            <div class="my-auto content-col">
                                <h5 class="text-blue fw-bold text-capitalize">Planning</h5>
                                <p>Tips for talking with aging parents</p>
                            </div>
                            <div class="my-auto me-5">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/family.png" class="float-end rounded" alt="">
                            </div>
                            <div class="corner-badge">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/Monitor-Heart-Beat.png" alt="">
                            </div>
                        </div>
                    </div>
                    <!-- Card 3 -->
                    <div class="col-lg-5 resource-card-main">
                        <div class="resource-card  py-3 px-3 py-lg-4 px-lg-4 position-relative d-flex gap-2 gap-lg-5">
                            <div class="my-auto content-col">
                                <h5 class="text-blue fw-bold text-capitalize">Finances</h5>
                                <p>Thinking about the cost of senior living communities</p>
                            </div>
                            <div class="my-auto me-5">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/family.png" class="float-end rounded" alt="">
                            </div>
                            <div class="corner-badge">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/Monitor-Heart-Beat.png" alt="">
                            </div>
                        </div>
                    </div>
                    <!-- Card 4 -->
                    <div class="col-lg-5 resource-card-main">
                        <div class="resource-card  py-3 px-3 py-lg-4 px-lg-4 position-relative d-flex gap-2 gap-lg-5">
                            <div class="my-auto content-col">
                                <h5 class="text-blue fw-bold text-capitalize">Family & Caregivers</h5>
                                <p>Care for the caregiver</p>
                            </div>
                            <div class="my-auto me-5">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/family.png" class="float-end rounded" alt="">
                            </div>
                            <div class="corner-badge">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/Monitor-Heart-Beat.png" alt="">
                            </div>
                        </div>

                    </div>
                    <!-- Card 5 -->
                    <div class="col-lg-5 resource-card-main">
                        <div class="resource-card  py-3 px-3 py-lg-4 px-lg-4 position-relative d-flex gap-2 gap-lg-5">
                            <div class="my-auto content-col">
                                <h5 class="text-blue fw-bold text-capitalize">Family & Caregivers</h5>
                                <p>Tips for successful visits with a loved one with Dementia</p>
                            </div>
                            <div class="my-auto me-5">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/family.png" class="float-end rounded" alt="">
                            </div>
                            <div class="corner-badge">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/Monitor-Heart-Beat.png" alt="">
                            </div>
                        </div>
                    </div>
                    <!-- Card 6 -->
                    <div class="col-lg-5 resource-card-main">
                        <div class="resource-card  py-3 px-3 py-lg-4 px-lg-4 position-relative d-flex gap-2 gap-lg-5 mb-4">
                            <div class="my-auto content-col">
                                <h5 class="text-blue fw-bold text-capitalize">Family & Caregivers</h5>
                                <p>Caring for your pet safely as you age</p>
                            </div>
                            <div class="my-auto me-5">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/family.png" class="float-end rounded" alt="">
                            </div>
                            <div class="corner-badge">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/Monitor-Heart-Beat.png" alt="">
                            </div>
                        </div>

                    </div>
                </div>
            </div>


            <!-- RIGHT COLUMN -->
            <div class="col-lg-3 mt-5 mt-lg-4 resources-category-col">
                <h3 class="fw-bold mb-2 font-medium mb-3">Resource Categories</h3>
                <ul class="list-unstyled categories-box m-0 resources-category-list">
                    <li class="py-4"><a href="#"><img src="<?php echo get_template_directory_uri(); ?>/assets/images/Disability-Sit-Cane-Streamline-Freehand-Duotone.png" class="pe-2">Assisted Living</a></li>
                    <li class="py-4"><a href="#"><img src="<?php echo get_template_directory_uri(); ?>/assets/images/Disability-Sit-Cane-Streamline-Freehand-Duotone.png" class="pe-2">Family & Caregivers</a></li>
                    <li class="py-4"><a href="#"><img src="<?php echo get_template_directory_uri(); ?>/assets/images/Disability-Sit-Cane-Streamline-Freehand-Duotone.png" class="pe-2">Finances</a></li>
                    <li class="py-4"><a href="#"><img src="<?php echo get_template_directory_uri(); ?>/assets/images/Disability-Sit-Cane-Streamline-Freehand-Duotone.png" class="pe-2">Health & Wellness</a></li>
                    <li class="py-4"><a href="#"><img src="<?php echo get_template_directory_uri(); ?>/assets/images/Disability-Sit-Cane-Streamline-Freehand-Duotone.png" class="pe-2">Independent Living</a></li>
                    <li class="py-4"><a href="#"><img src="<?php echo get_template_directory_uri(); ?>/assets/images/Disability-Sit-Cane-Streamline-Freehand-Duotone.png" class="pe-2">Let’s Get Started</a></li>
                    <li class="py-4"><a href="#"><img src="<?php echo get_template_directory_uri(); ?>/assets/images/Disability-Sit-Cane-Streamline-Freehand-Duotone.png" class="pe-2">Lifestyle</a></li>
                    <li class="py-4"><a href="#"><img src="<?php echo get_template_directory_uri(); ?>/assets/images/Disability-Sit-Cane-Streamline-Freehand-Duotone.png" class="pe-2">Memory Care</a></li>
                    <li class="py-4"><a href="#"><img src="<?php echo get_template_directory_uri(); ?>/assets/images/Disability-Sit-Cane-Streamline-Freehand-Duotone.png" class="pe-2">Planning</a></li>
                </ul>
            </div>
        </div>

    </div>
    </div>
</section>

<section class="py-5 git">
    <div class="container-fluid">
        <div class="row justify-content-center mb-5">
            <div class="col-12 text-center mb-4">
                <h2 class="fw-bold font-xl">Helpful Topics</h2>
                <div class="underline mx-auto"></div>
            </div>
        </div>
        <div class="row justify-content-center">
            <div class="col-lg-10">
                <div class="row">
                    <div class="col-md-6 col-lg-4 mb-5 px-4 px-4">
                        <div class="topic-card">
                            <div class="upper-content d-flex flex-column justify-content-center align-items-center">
                                <a href="">
                                    <h5 class="text-blue fw-bold text-capitalize has-arrow position-relative">Assisted Living</h5>
                                </a>
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/Disability-Sit-Cane-Streamline-Freehand-Duotone.png" class="">

                            </div>
                            <div class="below-content bg-light-blue p-3">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/family.png  " class="">
                                <div>
                                    <p class="text-blue mb-0">Latest Resource</p>
                                    <p class="mb-0">Is Personal Care or Assisted Living right for me or my loved one?</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-4 mb-5 px-4 px-4">
                        <div class="topic-card">
                            <div class="upper-content d-flex flex-column justify-content-center align-items-center">
                                <a href="">
                                    <h5 class="text-blue fw-bold text-capitalize has-arrow position-relative">Family & Caregivers</h5>
                                </a>
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/Disability-Sit-Cane-Streamline-Freehand-Duotone.png" class="" alt="">

                            </div>
                            <div class="below-content bg-light-blue p-3">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/family.png" class="" alt="">
                                <div>
                                    <p class="text-blue mb-0">Family & Caregivers</p>
                                    <p class="mb-0">Is Personal Care or Assisted Living right for me or my loved one?</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-4 mb-5 px-4 px-4">
                        <div class="topic-card">
                            <div class="upper-content d-flex flex-column justify-content-center align-items-center">
                                <a href="">
                                    <h5 class="text-blue fw-bold text-capitalize has-arrow position-relative">Health & Wellness</h5>
                                </a>
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/Disability-Sit-Cane-Streamline-Freehand-Duotone.png" class="">

                            </div>
                            <div class="below-content bg-light-blue p-3">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/family.png" class="">
                                <div>
                                    <p class="text-blue mb-0">Latest Resource</p>
                                    <p class="mb-0">Is Personal Care or Assisted Living right for me or my loved one?</p>
                                </div>
                            </div>
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-4 mb-5 px-4 px-4">
                        <div class="topic-card">
                            <div class="upper-content d-flex flex-column justify-content-center align-items-center">
                                <a href="">
                                    <h5 class="text-blue fw-bold text-capitalize has-arrow position-relative">Finances</h5>
                                </a>
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/Adoptation_Family.png" class="">

                            </div>
                            <div class="below-content bg-light-blue p-3">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/family.png" class="">
                                <div>
                                    <p class="text-blue mb-0">Latest Resource</p>
                                    <p class="mb-0">Is Personal Care or Assisted Living right for me or my loved one?</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-4 mb-5 px-4 px-4">
                        <div class="topic-card">
                            <div class="upper-content d-flex flex-column justify-content-center align-items-center">
                                <a href="">
                                    <h5 class="text-blue fw-bold text-capitalize has-arrow position-relative">Finances</h5>
                                </a>
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/Disability-Sit-Cane-Streamline-Freehand-Duotone.png" class="">

                            </div>
                            <div class="below-content bg-light-blue p-3">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/family.png" class="">
                                <div>
                                    <p class="text-blue mb-0">Latest Resource</p>
                                    <p class="mb-0">Is Personal Care or Assisted Living right for me or my loved one?</p>
                                </div>
                            </div>
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-4 mb-5 px-4 px-4">
                        <div class="topic-card">
                            <div class="upper-content d-flex flex-column justify-content-center align-items-center">
                                <a href="">
                                    <h5 class="text-blue fw-bold text-capitalize has-arrow position-relative">Finances</h5>
                                </a>
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/Disability-Sit-Cane-Streamline-Freehand-Duotone.png" class="">

                            </div>
                            <div class="below-content bg-light-blue p-3">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/family.png" class="">
                                <div>
                                    <p class="text-blue mb-0">Independent Living</p>
                                    <p class="mb-0">Is Personal Care or Assisted Living right for me or my loved one?</p>
                                </div>
                            </div>
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-4 mb-5 px-4 px-4">
                        <div class="topic-card">
                            <div class="upper-content d-flex flex-column justify-content-center align-items-center">
                                <a href="">
                                    <h5 class="text-blue fw-bold text-capitalize has-arrow position-relative">Finances</h5>
                                </a>
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/Disability-Sit-Cane-Streamline-Freehand-Duotone.png" class="">

                            </div>
                            <div class="below-content bg-light-blue p-3">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/family.png" class="">
                                <div>
                                    <p class="text-blue mb-0">Let’s Get Started...</p>
                                    <p class="mb-0">Is Personal Care or Assisted Living right for me or my loved one?</p>
                                </div>
                            </div>
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-4 mb-5 px-4 px-4">
                        <div class="topic-card">
                            <div class="upper-content d-flex flex-column justify-content-center align-items-center">
                                <a href="">
                                    <h5 class="text-blue fw-bold text-capitalize has-arrow position-relative">Finances</h5>
                                </a>
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/Disability-Sit-Cane-Streamline-Freehand-Duotone.png" class="">

                            </div>
                            <div class="below-content bg-light-blue p-3">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/family.png" class="">
                                <div>
                                    <p class="text-blue mb-0">Lifestyle</p>
                                    <p class="mb-0">Is Personal Care or Assisted Living right for me or my loved one?</p>
                                </div>
                            </div>
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-4 mb-5 px-4 px-4">
                        <div class="topic-card">
                            <div class="upper-content d-flex flex-column justify-content-center align-items-center">
                                <a href="">
                                    <h5 class="text-blue fw-bold text-capitalize has-arrow position-relative">Finances</h5>
                                </a>
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/Disability-Sit-Cane-Streamline-Freehand-Duotone.png" class="">

                            </div>
                            <div class="below-content bg-light-blue p-3">
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/family.png" class="">
                                <div>
                                    <p class="text-blue mb-0">Memory Care</p>
                                    <p class="mb-0">Is Personal Care or Assisted Living right for me or my loved one?</p>
                                </div>
                            </div>
                        </div>
                    </div>

                    
                </div>
            </div>
        </div>
</section>



<?php get_footer(); ?>